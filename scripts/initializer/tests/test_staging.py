...
class InstallationPlanTests(unittest.TestCase):
    def test_selects_only_installable_entries(self):
        entries = [
            _make_entry("specs/repo/", classification="framework-authoritative"),
        ...
        self.assertIn("specs/repo/", paths)
    ...
    def test_equality(self):
        e1 = [_make_entry("specs/repo/"), _make_entry("scripts/", classification="framework-support")]
        e2 = [_make_entry("specs/repo/"), _make_entry("scripts/", classification="framework-support")]
    ...
    def test_inequality(self):
        e1 = [_make_entry("specs/repo/")]
        e2 = [_make_entry("scripts/", classification="framework-support")]
        p1 = InstallationPlan(ClassifiedInventory(e1))
        p2 = InstallationPlan(ClassifiedInventory(e2))
...
class InstallationResultTests(unittest.TestCase):
    def test_to_dict_structure(self):
        ...
        result = InstallationResult(
            ...
            installed=[{"path": "specs/repo/", "classification": "framework-authoritative", "type": "directory"}],
            skipped=[],
            rejected=[],
        )
        d = result.to_dict()
...
class BuildInstallationPlanTests(unittest.TestCase):
    def test_from_classified_inventory(self):
        entries = [
            _make_entry("specs/repo/", classification="framework-authoritative"),
            _make_entry("scripts/", classification="framework-support"),
            _make_entry("derived/", classification="derived"),
        ]
        ci = ClassifiedInventory(entries)
...
class ValidateSourcePathTests(unittest.TestCase):
    def setUp(self):
        ...
        (self.root / "specs" / "repo").mkdir()

    def tearDown(self):
        shutil.rmtree(self.root, ignore_errors=True)

    def test_valid_path(self):
        validate_source_path(self.root, "specs/repo/", _make_entry("specs/repo/"))
...
class DestinationConflictTests(unittest.TestCase):
    ...
    def test_no_conflict(self):
        entries = [
            _make_entry("specs/repo/"),
            _make_entry("scripts/validate"),
        ]
    ...
    def test_nested_overlap_rejected(self):
        entries = [
            _make_entry("specs/"),
            _make_entry("specs/repo/manifest.json"),
        ]
        with self.assertRaises(StagingError):
            check_destination_conflicts(self.staging, entries)
...
class StageFrameworkTests(unittest.TestCase):
    ...
    def _create_framework_source(self):
        ...
        (self.source_root / "schemas").mkdir()
        (self.source_root / "schemas" / "repo-schema.json").write_text('{}')
        (self.source_root / "scripts").mkdir()
        (self.source_root / "scripts" / "validate").write_text("#!/bin/bash\necho ok")
        (self.source_root / "derived").mkdir()
        (self.source_root / "derived" / "README.md").write_text("# derived")
    ...
    def _make_classified_inventory(self) -> ClassifiedInventory:
        entries = [
            _make_entry("specs/repo/", classification="framework-authoritative"),
            _make_entry("schemas/", classification="framework-authoritative"),
            _make_entry("scripts/validate", classification="framework-support"),
            _make_entry("derived/", classification="derived"),
            _make_entry("docs/overview/", classification="product-instance", installable=False,
                        exclusion_rationale="product instance"),
            _make_entry(".github/", classification="installed-adapter", installable=False,
    ...
    def test_stages_installable_framework_material(self):
        ...
        self.assertIn("specs/repo/", installed_paths)
        self.assertIn("schemas/", installed_paths)
        self.assertIn("scripts/validate", installed_paths)
        self.assertIn("derived/", installed_paths)
