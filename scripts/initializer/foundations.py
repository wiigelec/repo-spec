...
def _product_manifest_content(plan: FoundationPlan, slug: str, governing_issue: str) -> str:
    manifest = {
        ...
        "derived_artifacts": [
            {"type": "markdown", "path": f"derived/specs/product/manifest.md"},
        ],
    }
...
def _readme_discoverability_content(slug: str, product_id: str) -> str:
    return "\n".join([
        ...
        f"- [Product manifest](specs/product/manifest.json)",
        "",
    ])
...
def establish_product_foundations(
    ...
    # Product manifest
    manifest_path = staging / "specs" / "product" / "manifest.json"
    _safe_write(
        manifest_path,
    ...
    level_roots = [
        "specs/product/level-0/",
        "specs/product/level-1/",
        "specs/product/level-2/",
        "specs/product/level-3/",
    ]
    ...
    # Product overview README update (products README under specs/product/)
    specs_product_readme = staging / "specs" / "product" / "README.md"
