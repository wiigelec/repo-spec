...
PROFILE_MANIFEST_PATH = "profiles/github/manifest.json"
...
def load_profile_manifest(repo_root: Path) -> dict:
    ...
    if manifest.get("source_root") != "profiles/github/":
        fail(f"invalid profile manifest: {PROFILE_MANIFEST_PATH}")
...
def check_profile_freshness(repo_root: Path) -> None:
    ...
    for target_rel, content in rendered:
        ...
        if not target.exists() or target.read_text() != content:
            source_rel = target_rel.replace(".github/", "profiles/github/") if target_rel.startswith(".github/") else target_rel
