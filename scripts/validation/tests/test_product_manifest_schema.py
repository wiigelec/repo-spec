...
def assert_valid(schema: dict, name: str) -> None:
    validate_instance(load_fixture(name), schema, f"scripts/validation/tests/fixtures/product-manifest/{name}", schema)


def assert_invalid(schema: dict, name: str, fragment: str) -> None:
    expect_failure(
        name,
        lambda: validate_instance(load_fixture(name), schema, f"scripts/validation/tests/fixtures/product-manifest/{name}", schema),
        fragment,
    )
