...
| Every I1-owned key has one baseline classification (preserve/repair/replace/implement) with cited source/test evidence | I1 subset reconciliation from the complete B0 matrix; counts reconcile to ownership for `product.initializer-level-0`, `product.initialization-request`, `product.source-revision-identity`, `product.destination`, `product.product-identity`, `product.material-manifest`, `product.git-object-identity`, `product.destination-preflight`, `product.request-intake`, `product.source-material-resolution` |
