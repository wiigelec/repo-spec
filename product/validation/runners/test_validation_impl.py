...
def _run_source_development_tests(repo_root: Path) -> None:
    ...
    from validation.tests.self.mutation_tests import run_product_mutation_tests

    run_product_mutation_tests(repo_root)


# Product conformance-driven test execution.
#!/usr/bin/env python3
...
def main(argv: list[str]) -> int:
    ...
    if args:
        print(f"validation test error: unknown mode: {args[0]}", file=sys.stderr)
        return 1
