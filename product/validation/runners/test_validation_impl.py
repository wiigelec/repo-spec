...
from validation.tests.portable_self_tests import run_product_portable_self_tests


def _run_source_development_tests(repo_root: Path) -> None:
    source_suite = repo_root / "product/validation/tests/mutation_tests.py"
    if not source_suite.is_file():
        return

    from validation.tests.mutation_tests import run_product_mutation_tests
