...
- `INIT-RPT-006`: The report `completion_status` shall be one of: `failed` (pre-promotion failure or indeterminate promotion) or `promoted-with-finalization-error` (promotion committed but post-promotion cleanup failed). `schema_version` shall be the constant string `1`. A `promoted-with-finalization-error` completion_status shall not be misreported as a pre-promotion failure; the repository is successfully promoted.
...
- `INIT-RPT-012`: When post-promotion cleanup fails after successful promotion, the execution report shall have completion_status promoted-with-finalization-error. The report shall record the success-finalization stage status as failed with the cleanup failure reason in the errors array, and shall record the promotion stage status as completed. The execution report shall reference the promoted destination path and shall not misreport the repository as unpromoted. Under cleanup failure, the execution report stages shall represent: promotion as completed, success-finalization as failed, and all prior stages as completed. The execution report and staging-state record shall agree on the terminal outcome: both shall record promotion as committed and success-finalization as failed.
