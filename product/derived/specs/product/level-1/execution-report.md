...
- `INIT-RPT-006`: The report `completion_status` shall be one of: `failed` (pre-promotion failure or indeterminate promotion) or `promoted-with-finalization-error` (promotion committed but post-promotion cleanup failed). `schema_version` shall be the constant string `1`.
