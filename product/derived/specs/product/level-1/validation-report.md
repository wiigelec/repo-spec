...
- `INIT-VR-013`: The validation report shall not include a check that recursively validates the report itself. Fingerprint linkage, digest linkage, check ordering, check uniqueness, report schema validation, and serialization are not ordinary profile checks; they are validation-component postconditions enforced during report finalization before promotion.
- `INIT-VR-014`: Report finalization is a deterministic assembly step that occurs after all Phase 1 and Phase 2 checks are recorded. Finalization shall: (a) verify that the request_fingerprint in the report matches the request_fingerprint in the staging-state record, (b) verify that the repository_content_digest in the report matches the expected digest from the staging-state record and the digest computed during the output.repository-digest-match check, (c) verify that the checks array contains exactly one entry per profile check_id in ascending order-number order with no missing or duplicate entries, (d) calculate overall_status as pass only when every required Phase 1 and Phase 2 check has status passed, (e) validate the completed report against the validation-report schema, and (f) serialize the final report deterministically.
- `INIT-VR-015`: Report finalization shall complete before promotion begins. Promotion must not proceed without a finalized report whose overall_status is pass. Finalization failure (linkage mismatch, ordering violation, schema failure, or serialization error) shall prevent promotion and shall be recorded as a pre-promotion failure with the staging transaction root preserved.
