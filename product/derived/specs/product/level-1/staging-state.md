...
- `INIT-STA-011`: When promotion_outcome is promoted and post-promotion staging-root removal fails, the staging-state record may record an additional `cleanup_failure` field that captures the cleanup failure reason as a string. The presence of a `cleanup_failure` field does not change the promotion outcome: the repository remains promoted. The staging-state record shall treat `cleanup_failure` as an optional field present only when cleanup after successful promotion fails.
- `INIT-STA-012`: The staging-state record shall reject unknown fields and shall treat every field of this specification as required with no optional fields, except where a field is explicitly declared optional (such as `cleanup_failure`).
