...
- `INIT-FIN-007`: The component shall validate every installed path against the output inventory (product.initializer-output-inventory-v1): every installed path with producer framework-installation must match an inventory entry, the operation must match, and the mode must match the inventory declaration.
