...
def actual_derived_markdown_paths(repo_root: Path) -> set[str]:
    paths: set[str] = set()
    for derived_root in (repo_root / "repo/derived/specs/repo", repo_root / "product/derived/specs/product"):
...
def check_generated_outputs(repo_root: Path) -> None:
    rendered = render_all_detailed(repo_root)
    expected_markdown_paths = {path for path in rendered if path.endswith(".md") and (path.startswith("repo/derived/specs/") or path.startswith("product/derived/specs/"))}
