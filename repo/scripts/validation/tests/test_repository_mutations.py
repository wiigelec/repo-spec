...
def run_repository_mutations(repo_root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="repo-spec-validation-") as temp_root_name:
        ...
        expect_failure("product root contamination", lambda: validate_repo(temp_repo), "undeclared JSON content under product/specs/product/")
        ...
        for level_name in ["level-0", "level-1", "level-2", "level-3"]:
            ...
            expect_failure(f"product root contamination in {level_name}", lambda: validate_repo(temp_repo), "undeclared JSON content under product/specs/product/")
