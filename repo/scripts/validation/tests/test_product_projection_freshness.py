...
def run_product_projection_freshness_tests(repo_root: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="repo-spec-validation-") as temp_root_name:
        ...
        expect_failure(
            ...
            "stale generated document: source product/specs/product/level-1/primitive.json -> output product/derived/specs/product/primitive.md",
        )
        ...
        expect_failure(
            ...
            lambda: check_generated_document_freshness(temp_repo),
            "missing derived markdown: product/derived/specs/product/component.md",
        )
        ...
        mutate_json(
            ...
            lambda spec: spec["derived_artifacts"].__setitem__(0, {"type": "markdown", "path": "product/derived/specs/product/validation.md"}) or spec,
        )
        expect_failure(
            "repository projection in product root",
            lambda: validate_repo(temp_repo),
            "missing derived markdown: product/derived/specs/product/validation.md; orphaned derived markdown: repo/derived/specs/repo/validation.md",
        )

        temp_repo = build_product_repo(repo_root, temp_root, 7)
        primitive_doc = temp_repo / "product/derived/specs/product/primitive.md"
        primitive_doc.write_text(primitive_doc.read_text().replace("Generated by `repo/scripts/generate-docs`", "Generated by `repo/scripts/generate-docs` (tampered)", 1))
        expect_failure(
        ...
        expect_failure(
            "order instability",
            lambda: check_generated_document_freshness(temp_repo),
            "stale generated document: source product/specs/product/level-3/orchestration.json -> output product/derived/specs/product/orchestration.md",
        )
