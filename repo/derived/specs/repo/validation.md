...
- `REPO-VAL-001`: The repository shall provide `repo/scripts/validate` as the repository-owned leaf validation entry point, `product/scripts/validate` as the product-owned leaf validation entry point, and `scripts/validate` as the aggregate repository-wide validation entry point. Each leaf validation entry point shall validate only its owned tree, and the aggregate entry point shall run both leaf validators plus the repository-owned and product-owned validation mutation/self-test suites.
...
- `REPO-VAL-020`: Each leaf validation entry point shall enforce the machine-readable metadata envelope and required section structure for governed overview, decomposition, and planning artifacts within its owned tree.
...
- `REPO-VAL-022`: Each leaf validation entry point shall enforce type-specific content-area inventories for governed overview, decomposition, and planning artifacts within its owned tree so the declared chunk roles cover the subordinate chunks exactly once.
...
- `REPO-VAL-036`: The hosting-platform issue-body field-policy boundary, rather than either repository-tree leaf validation entry point, shall reject a governing issue declared as product-artifact implementation when the issue does not cite a canonical implementation-plan path, at least one manifest-listed accepted product specification, the exact accepted default-branch base, and predecessor implementation issue and revision evidence. Repository and product leaf validation shall remain repository-local and shall not inspect remote issue bodies.
